<template>
    <div class="flex justify-between mb-4">
        <h1 class="text-2xl font-bold mb-4">Medical Technicians</h1>
        <button  class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded w-1/5" @click="showAddModal = true">Add Medical Technician</button>
    </div>
...
export default {
  ...
  methods: {
    addUser() {
      axios.post('http://localhost:3501/api/medical-technicians', this.newUser)
        .then(response => {
     ...
     saveUser() {
      axios.put(`http://localhost:3501/api/medical-technicians/${this.editingUser._id}`, this.editingUser)
        .then(() => {
    ...
    deleteUser(user) {
      if (
        ...
        axios
          .delete(`http://localhost:3501/api/medical-technicians/${user._id}`)
          .then(() => {
    ...
    getUsers() {
      axios
        .get("http://localhost:3501/api/medical-technicians/")
        .then((response) => {
