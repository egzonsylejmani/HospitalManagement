<template>
    <div class="flex justify-between mb-4">
        ...
        <button  class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded w-1/5" @click="showAddModal = true">Add Technician</button>
    </div>
...
export default {
  ...
  methods: {
    addUser() {
      axios.post('http://localhost:3501/api/technicians', this.newUser)
        .then(response => {
     ...
     saveUser() {
      axios.put(`http://localhost:3501/api/technicians/${this.editingUser._id}`, this.editingUser)
        .then(() => {
